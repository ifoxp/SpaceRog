...
public class EnemyDestroyers : NetworkBehaviour
{
    ...
    private void Start()
    {
        ...
        LookAtPlayer(1000);  // Направляємо корабель на гравця при старті
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (goToBords == 2)
        {
            LookAtPlayer(1000);                  // Направляємо корабель на гравця
            goToBords = 1;
        }
    ...
    private void LookAtPlayer(int speed)
    {
        ...
        Quaternion rotation = Quaternion.Euler(0f, 0f, angle);     // Обертання по Z-осі
        enemyR.rotation = Quaternion.RotateTowards(enemyR.rotation, rotation, speedRotate * speed * Time.deltaTime);   // Моментальне обертання корабля
    }
